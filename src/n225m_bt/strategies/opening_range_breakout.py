"""Opening range breakout strategy with versioned session calendar rules."""
...
@component(
    ...
    kind="strategy",
    summary="Opening range breakout with session clock, fixed range, strict threshold cross and tick bracket",
    tags=["breakout", "opening_range", "intraday"],
    uses=["session_clock", "fixed_range", "threshold_cross", "tick_bracket"],
)
class OpeningRangeBreakout:
    """Day-session opening range breakout strategy with strict buffers and tick brackets."""
    ...
    def __init__(
        ...
        if min(opening_minutes, entry_window_minutes, tick_size, max_entry_signals_per_session) <= 0:
            raise ValueError(
                "opening_minutes, entry_window_minutes, tick_size, and max_entry_signals_per_session must be positive"
            )
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> Signal | None:
        ...
        if self.current_range is None or self.range_end_ts is None or self.entry_end_ts is None:
            return None
...
def create_strategy(parameters: dict[str, Any]) -> OpeningRangeBreakout:
    """Create a fresh instance of OpeningRangeBreakout for one parameter set."""
    return OpeningRangeBreakout(**parameters)
