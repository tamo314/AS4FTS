...
## 仮説
日中の途中までの一方向への価格変化が大きいとき、引け前には継続または巻き戻しの特徴があるか。
...
## この戦略の売買規則
1. 適用期間付きのsession_closeからminutes_before_closeを引いた時点で一度判断します。固定の14時等に置換せず、ザラ場終了とセッション終値の意味を混同しません。
2. 日中始値Od、判断時点の確定終値Ct、その時点までの高値Hと安値Lを使い、W=H-L、日中方向性=abs(Ct-Od)/Wを計算します。当日終了後に確定するH,Lは使いません。
3. 日中方向性>=min_day_directionなら、continuationはsign(Ct-Od)方向、reversalは逆方向を候補とし、directionと一致するときにシグナルを出します。W=0または方向0なら見送ります。
4. 判断予定の足がない場合は、後の足をその時点の足とみなしません。セッション終わりに基づく既存の新規停止・強制決済を維持します。翌セッションへ判断や建玉を意図的に持ち越しません。
部品候補: SessionClock、途中経過を含むSessionOHLC、ScaledBracket。H01/H05で作った機能を再利用します。
評価焦点: 判断時間と日中方向性の条件別に継続/反転を比較し、特定期間の偏りを調べます。

## 固定する初期探索仕様
```yaml
factory: n225m_bt.strategies.preclose_response:create_strategy
